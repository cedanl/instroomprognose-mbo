...
applications_conversion %>%
  ...
  labs(title = "Conversiepercentage per Type Meervoudige Aanmelding",
...
ggplot(applications_count_distribution %>% filter(schooljaar %in% c(2023, 2024)), 
       aes(x = students_pct, 
           fill = factor(schooljaar, levels = c("2024", "2023")),
...
ggplot(conversion_by_count %>% filter(schooljaar %in% c(2023, 2024)), 
       aes(y = reorder(factor(applications_total_number), -as.numeric(applications_total_number)), 
           ...
           fill = factor(schooljaar, levels = c("2024", "2023")))) +  # Reverse year order
  geom_col(position = "dodge") +
...
# Visualiseer conversiepercentages van top en laagste opleidingen (horizontaal)
plot_ly() %>%
  # Voeg laagste 5 toe met tekstlabels aan buitenkant
  add_trace(
    data = bottom5_programs,
    y = ~reorder(Opleidingsnaam, -as.numeric(factor(Opleidingsnaam, levels = ordered_programs$Opleidingsnaam))),
    x = ~conversion_rate, 
    type = "bar",
    name = "Laagste 5",
  ...
  # Voeg top 5 toe met tekstlabels aan binnenkant
  add_trace(
    data = top5_programs,
    y = ~reorder(Opleidingsnaam, -as.numeric(factor(Opleidingsnaam, levels = ordered_programs$Opleidingsnaam))),
    x = ~conversion_rate, 
  ...
  layout(
    title = "Conversiepercentages Top 5 en Laagste 5 Opleidingen",
    yaxis = list(title = ""),
    xaxis = list(title = "Conversiepercentage (%)"),
    margin = list(l = 150, r = 50),  # Extra marge rechts voor outside labels
    showlegend = TRUE,
    barmode = "group"
  )
```
...
# Visualiseer conversiepercentages van top en laagste instellingen (horizontaal)
plot_ly() %>%
  # Voeg laagste 5 toe met tekstlabels aan buitenkant
  add_trace(
    data = bottom5_institutions,
    y = ~reorder(school, -as.numeric(factor(school, levels = ordered_institutions$school))),
    x = ~conversion_rate, 
    type = "bar",
    name = "Laagste 5",
    marker = list(color = "#FF9800"),
  ...
  layout(
    title = "Conversiepercentages Top 5 en Laagste 5 Instellingen",
    yaxis = list(title = ""),
    xaxis = list(title = "Conversiepercentage (%)"),
    margin = list(l = 150, r = 50),  # Extra marge rechts voor outside labels
    showlegend = TRUE,
