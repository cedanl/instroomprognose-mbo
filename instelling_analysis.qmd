...
institution_data <- load_and_enrich_applications(config_env = "cambo") %>%
  filter_applications(
    years = params$years,
...
# Calculate market share for this institution
market_share_data <- calculate_market_share_by_postcode(
    institution_data, 
...
# Summary statistics
market_summary <- market_share_data %>%
    group_by(schooljaar) %>%
    summarise(
        postcodes_with_students = sum(institution_students > 0),
        total_postcodes = n(),
        avg_market_share = mean(market_share_pct[institution_students > 0], na.rm = TRUE),
        max_market_share = max(market_share_pct, na.rm = TRUE),
        total_students = sum(institution_students),
        .groups = "drop"
    )

cat("Geografische analyse voor", params$school_name, "\n")
cat("Aantal postcodes in analyse:", nrow(market_share_data), "\n")
cat("Schooljaren:", paste(unique(market_share_data$schooljaar), collapse = ", "), "\n")
...
# Show market share summary
market_summary %>%
    gt() %>%
    tab_header(
        title = paste("Marktaandeel Samenvatting:", params$school_name),
    ...
    cols_label(
        ...
        avg_market_share = "Gemiddeld marktaandeel (%)",
        max_market_share = "Maximaal marktaandeel (%)",
    ...
    fmt_number(
        columns = c(avg_market_share, max_market_share),
        decimals = 1
    )
...
# Create maps for each year if we have geographic data and market share data
if (nrow(market_share_data) > 0 && file.exists("data/reference/cbs_pc4_2024_v1.gpkg")) {
    
    for (year in params$years) {
        cat("### Marktaandeel Kaart", year, "\n\n")
        
        # Create map for this year
        year_map <- create_market_share_map(
    ...
    cat("Geen geografische data beschikbaar voor kaarten.\n\n")
}
...
```{r}
#| label: top-postcodes-per-year-table

# Find top postcodes per year
if (nrow(market_share_data) > 0) {
    top_postcodes_by_year <- market_share_data %>%
        filter(institution_students > 0) %>%
        group_by(schooljaar) %>%
        arrange(desc(institution_students)) %>%
        slice_head(n = 10) %>%
        ungroup() %>%
        select(schooljaar, postcode_4, institution_students, total_students, market_share_pct)
    
    # Display top postcodes table
    top_postcodes_by_year %>%
        gt() %>%
        tab_header(
            title = paste("Top 10 Postcodes per Jaar voor", params$school_name),
            subtitle = "Gerangschikt op aantal studenten per jaar"
        ) %>%
        cols_label(
            schooljaar = "Jaar",
            postcode_4 = "Postcode",
            institution_students = "Studenten",
            total_students = "Totaal MBO Studenten",
            market_share_pct = "Marktaandeel (%)"
        ) %>%
        fmt_number(
            columns = c(institution_students, total_students),
            decimals = 0
        ) %>%
        fmt_number(
            columns = market_share_pct,
            decimals = 1
        ) %>%
        tab_style(
            style = cell_fill(color = "#f0f8ff"),
            locations = cells_body(rows = market_share_pct > 50)
        ) %>%
        tab_style(
            style = cell_fill(color = "#fff2e6"),
            locations = cells_body(rows = schooljaar == max(params$years))
        )
} else {
    cat("Geen postcodes met studenten gevonden voor", params$school_name)
}
...
if (nrow(market_share_data) > 0 && file.exists("data/reference/cbs_pc4_2024_v1.gpkg")) {
    ...
    recent_year <- max(market_share_data$schooljaar)
    market_population <- market_share_data %>%
    ...
    if (nrow(market_population) > 10) {
        ...
        market_population %>%
            ggplot(aes(x = aantal_inwoners, y = market_share_pct)) +
            ...
            theme_minimal() +
            labs(
                title = paste("Marktaandeel vs Bevolkingsdichtheid -", params$school_name),
                subtitle = paste("Schooljaar", recent_year, "- Grootte punt = aantal studenten"),
                x = "Aantal Inwoners per Postcode (log schaal)",
                y = "Marktaandeel (%)",
                caption = "Trend lijn toont gemiddelde relatie tussen bevolking en marktaandeel"
            )
        
        cat("\n### Analyse Marktaandeel vs Bevolking\n\n")
        
        # Calculate correlation
        correlation <- cor(log10(market_population$aantal_inwoners), 
                          market_population$market_share_pct, 
                          use = "complete.obs")
        
        cat("**Correlatie tussen log(bevolking) en marktaandeel:**", round(correlation, 3), "\n\n")
        
        # Find outliers - high market share in small populations
        high_efficiency <- market_population %>%
            filter(market_share_pct > 25, aantal_inwoners < 5000) %>%
            arrange(desc(market_share_pct)) %>%
            head(5)
        
        if (nrow(high_efficiency) > 0) {
            cat("**Hoog marktaandeel in kleine gebieden:**\n")
            for (i in 1:nrow(high_efficiency)) {
                cat("-", high_efficiency$postcode_4[i], 
                    "(", high_efficiency$market_share_pct[i], "% marktaandeel,", 
                    scales::comma(high_efficiency$aantal_inwoners[i]), "inwoners)\n")
            }
        }
        
    } else {
        cat("Onvoldoende data voor marktaandeel vs bevolking analyse.\n")
    }
} else {
...
if (nrow(market_share_data) > 0) {
    ...
    total_postcodes_with_students <- sum(market_share_data$institution_students > 0)
    high_share_postcodes <- sum(market_share_data$market_share_pct > 25, na.rm = TRUE)
    dominant_postcodes <- sum(market_share_data$market_share_pct > 50, na.rm = TRUE)
    
    # Most recent year stats
    recent_year <- max(market_share_data$schooljaar)
    recent_data <- market_share_data %>% filter(schooljaar == recent_year)
    
    cat("**Geografische Spreiding (", recent_year, "):**\n")
    cat("- Postcodes met studenten:", sum(recent_data$institution_students > 0), "\n")
    cat("- Postcodes met >25% marktaandeel:", sum(recent_data$market_share_pct > 25, na.rm = TRUE), "\n")
    cat("- Postcodes met >50% marktaandeel:", sum(recent_data$market_share_pct > 50, na.rm = TRUE), "\n")
    ...
    if (nrow(top_postcode) > 0) {
        cat("- Belangrijkste postcode:", top_postcode$postcode_4, 
            ...
            round(top_postcode$market_share_pct, 1), "% marktaandeel)\n")
    }
    
    cat("\n**Strategische Kansen:**\n")
    
    # Find high-volume, low-share postcodes (opportunities)
    opportunities <- recent_data %>%
        filter(total_students >= 10, market_share_pct < 20, market_share_pct > 0) %>%
        arrange(desc(total_students)) %>%
        head(5)
    
    if (nrow(opportunities) > 0) {
        cat("- Groeikansen in postcodes met veel MBO-studenten maar laag marktaandeel:\n")
        for (i in 1:nrow(opportunities)) {
            cat("  -", opportunities$postcode_4[i], 
                "(", opportunities$total_students[i], "totaal studenten,", 
                round(opportunities$market_share_pct[i], 1), "% marktaandeel)\n")
        }
    }
} else {
