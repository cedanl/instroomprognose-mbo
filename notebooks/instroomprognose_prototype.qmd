...
Voor inzicht in de geografische spreiding van studenten analyseren we waar studenten vandaan komen op basis van postcode. We focussen op MBO Amersfoort om hun marktaandeel per postcode te visualiseren voor 2023 en 2024.
...
market_share_analysis <- total_students_by_postcode %>%
    left_join(mbo_students_by_postcode, by = c("schooljaar", "postcode_4")) %>%
    mutate(
        mbo_amersfoort_students = replace_na(mbo_amersfoort_students, 0),
        market_share_pct = (mbo_amersfoort_students / total_students) * 100
    ) %>%
    filter(total_students >= 5)  # Only postcodes with at least 5 total students

cat("Analyse van marktaandeel MBO Amersfoort per postcode\n")
cat("Aantal postcodes in analyse:", nrow(market_share_analysis), "\n")
cat("Schooljaren:", paste(unique(market_share_analysis$schooljaar), collapse = ", "), "\n")
...
market_share_summary <- market_share_analysis %>%
    group_by(schooljaar) %>%
    summarise(
        postcodes_with_students = sum(mbo_amersfoort_students > 0),
        total_postcodes = n(),
        avg_market_share = mean(market_share_pct[mbo_amersfoort_students > 0], na.rm = TRUE),
...
market_share_summary %>%
    gt() %>%
    tab_header(
        title = "MBO Amersfoort Marktaandeel Samenvatting",
...
# Join market share data with geographic data for 2023
market_share_2023 <- market_share_analysis %>%
    filter(schooljaar == 2023) %>%
    mutate(postcode = as.numeric(postcode_4)) %>%
    select(-postcode_4)

geo_market_2023 <- postal_code_geo_wgs84 %>%
    left_join(market_share_2023, by = "postcode") %>%
    mutate(
        market_share_pct = replace_na(market_share_pct, 0),
        has_mbo_students = replace_na(mbo_amersfoort_students, 0) > 0
    )

# Join market share data with geographic data for 2024
market_share_2024 <- market_share_analysis %>%
    filter(schooljaar == 2024) %>%
    mutate(postcode = as.numeric(postcode_4)) %>%
    select(-postcode_4)

geo_market_2024 <- postal_code_geo_wgs84 %>%
    left_join(market_share_2024, by = "postcode") %>%
    mutate(
        market_share_pct = replace_na(market_share_pct, 0),
        has_mbo_students = replace_na(mbo_amersfoort_students, 0) > 0
    )
...
# Create color scale function for market share
create_market_share_colors <- function(market_share) {
    ...
    return(color_palette(market_share))
}

# Map for 2023
map_2023 <- leaflet(geo_market_2023) %>%
    addProviderTiles(providers$CartoDB.Positron) %>%
    addPolygons(
        fillColor = ~create_market_share_colors(market_share_pct),
        fillOpacity = 0.8,
        color = "white",
        weight = 0.5,
        popup = ~paste0(
            ...
            "<b>Totaal studenten:</b> ", replace_na(total_students, 0), "<br>",
...
# Display 2023 map
cat("## MBO Amersfoort Marktaandeel per Postcode - 2023\n\n")
map_2023
...
# Map for 2024
map_2024 <- leaflet(geo_market_2024) %>%
    addProviderTiles(providers$CartoDB.Positron) %>%
    addPolygons(
        fillColor = ~create_market_share_colors(market_share_pct),
        fillOpacity = 0.8,
...
# Display 2024 map
cat("## MBO Amersfoort Marktaandeel per Postcode - 2024\n\n")
map_2024
...
top_postcodes_comparison <- bind_rows(
    ...
    select(jaar, postcode_4, mbo_amersfoort_students, total_students, market_share_pct) %>%
    arrange(desc(mbo_amersfoort_students))
...
top_postcodes_comparison %>%
    ...
    fmt_number(
        columns = c(mbo_amersfoort_students, total_students),
        decimals = 0
    ) %>%
    fmt_number(
...
De geografische analyse van MBO Amersfoort toont interessante patronen:

1. **Geografische Concentratie**: MBO Amersfoort heeft een duidelijke regionale focus met hogere marktaandelen in de directe omgeving
2. **Marktpositie**: Het marktaandeel varieert significant per postcode, van 0% tot over de 50% in sommige gebieden
3. **Jaarlijkse Vergelijking**: Door 2023 en 2024 te vergelijken kunnen we trends in geografische spreiding identificeren
4. **Strategische Inzichten**: De kaarten tonen kansen voor uitbreiding in gebieden met lage marktaandelen maar hoge studentenaantallen

Deze analyse kan gebruikt worden voor:
- Gerichte marketingcampagnes in ondervertegenwoordigde gebieden
- Identificatie van concurrentieposities per regio
- Optimalisatie van locatiestrategie en bereikbaarheid
- Benchmark tegen andere MBO-instellingen in dezelfde regio
