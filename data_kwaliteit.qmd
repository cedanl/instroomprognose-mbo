...
ggplot(schooljaar_comparison, aes(x = factor(schooljaar_afgeleid), y = aantal, fill = factor(schooljaar))) +
    ...
    labs(
        title = "Verdeling van afgeleid schooljaar versus originele schooljaar",
        subtitle = "Op basis van created_date",
        x = "Afgeleid schooljaar",
        y = "Aantal aanmeldingen (per status)",
        fill = "Originele schooljaar"
    ) +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
if (knitr::is_html_output() | interactive()) {
  ...
  processmapR::export_map(filtered_graph, file_name = map_file)
  knitr::include_graphics(map_file)
}
```
